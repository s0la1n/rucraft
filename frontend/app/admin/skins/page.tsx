...
import { moderationApi, ModerationRequest, getBackendBaseUrl } from "@/lib/api";

// Функция для получения URL изображения
function getImageUrl(imagePath: string | null): string {
  if (!imagePath) return '/placeholder-skin.png';
  const filename = imagePath.split('/').pop() || '';
  return `${getBackendBaseUrl()}/content-image/skins/${encodeURIComponent(filename)}`;
}
...
export default function AdminSkinsPage() {
  ...
  const loadRequests = async (page = 1, status = "") => {
    try {
      setLoading(true);
      setError(null);
      const response = await moderationApi.getSkins({ per_page: 12, page, status });
  ...
  const handleApprove = async (id: number) => {
    try {
      setActionLoading(id);
      await moderationApi.approve(id, adminComment || undefined);
      await loadRequests(currentPage, statusFilter);
      setSelectedRequest(null);
      setAdminComment("");
    } catch (err) {
      alert(err instanceof Error ? err.message : "Не удалось одобрить скин");
    } finally {
  ...
  const handleReject = async (id: number) => {
    ...
    try {
      setActionLoading(id);
      await moderationApi.reject(id, adminComment);
      await loadRequests(currentPage, statusFilter);
      setSelectedRequest(null);
      setAdminComment("");
    } catch (err) {
      alert(err instanceof Error ? err.message : "Не удалось отклонить скин");
    } finally {
      setActionLoading(null);
    }
  };

  const openModal = (request: ModerationRequest) => {
  ...
  return (
    <RequireAuth adminOnly>
      <div className="page-content">
        ...
        <PageSection title="Модерация скинов">
          {/* Фильтры */}
          ...
          <div className="skins-grid-pages" style={{ display: "flex", flexWrap: "wrap", gap: "20px", justifyContent: "flex-start" }}>
            {requests.map((request) => {
              const imageUrl = getImageUrl(request.skin_texture_file);
              return (
                <article
                  key={request.id}
                ...
                >
                  ...
                  <p className="skin-card-category" style={{ fontSize: "12px" }}>{request.category}</p>
                ...
                <div style={{ display: "flex", gap: "20px", marginBottom: "20px" }}>
                  <div style={{ flex: "0 0 200px" }}>
                    <div style={{ width: "200px", height: "200px", position: "relative", background: "#f0f0f0", borderRadius: "8px" }}>
                      <img
                        src={getImageUrl(selectedRequest.skin_texture_file)}
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p><strong>Модель:</strong> {selectedRequest.model}</p>
