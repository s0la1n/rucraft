...
import { seedModerationApi, SeedModerationRequest, getBackendBaseUrl } from "@/lib/api";

// Функция для получения URL изображения
function getImageUrl(imagePath: string | null): string {
  if (!imagePath) return '/placeholder-skin.png';
  const filename = imagePath.split('/').pop() || '';
  return `${getBackendBaseUrl()}/content-image/seeds/${encodeURIComponent(filename)}`;
}
...
export default function AdminSeedsPage() {
  const [requests, setRequests] = useState<SeedModerationRequest[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [currentPage, setCurrentPage] = useState(1);
  const [lastPage, setLastPage] = useState(1);
  const [statusFilter, setStatusFilter] = useState("");
  const [selectedRequest, setSelectedRequest] = useState<SeedModerationRequest | null>(null);
  ...
  const loadRequests = async (page = 1, status = "") => {
    try {
      setLoading(true);
      setError(null);
      const response = await seedModerationApi.getSeeds({ per_page: 12, page, status });
  ...
  const handleApprove = async (id: number) => {
    try {
      setActionLoading(id);
      await seedModerationApi.approve(id, adminComment || undefined);
      await loadRequests(currentPage, statusFilter);
      setSelectedRequest(null);
      setAdminComment("");
    } catch (err) {
      alert(err instanceof Error ? err.message : "Не удалось одобрить сид");
    } finally {
  ...
  const handleReject = async (id: number) => {
    ...
    try {
      setActionLoading(id);
      await seedModerationApi.reject(id, adminComment);
      await loadRequests(currentPage, statusFilter);
      setSelectedRequest(null);
      setAdminComment("");
    } catch (err) {
      alert(err instanceof Error ? err.message : "Не удалось отклонить сид");
    } finally {
      setActionLoading(null);
    }
  };

  const openModal = (request: SeedModerationRequest) => {
  ...
  return (
    <RequireAuth adminOnly>
      <div className="page-content">
        ...
        <PageSection title="Модерация сидов">
          {/* Фильтры */}
          ...
          <div className="skins-grid-pages" style={{ display: "flex", flexWrap: "wrap", gap: "20px", justifyContent: "flex-start" }}>
            {requests.map((request) => {
              ...
              console.log('[SeedsModeration] Request ID:', request.id, 'image:', request.image, 'resolved URL:', imageUrl);
              return (
                <article
                  key={request.id}
                ...
                >
                  <div className="skin-card-image-wrap" style={{ overflow: "hidden" }}>
                    <div className="skin-card-canvas-placeholder" style={{ position: "relative", width: "100%", paddingTop: "100%", overflow: "hidden" }}>
                      <img
                        ...
                        loading="lazy"
                        onError={(e) => {
                          console.error('[SeedsModeration] Image load failed:', imageUrl);
                  ...
                  <p className="skin-card-category" style={{ fontSize: "12px" }}>Сид: {request.seed_number}</p>
              ...
              <div className="modal-body">
                <div style={{ display: "flex", gap: "20px", marginBottom: "20px" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p><strong>Сид:</strong> {selectedRequest.seed_number}</p>
                    <p><strong>Версия:</strong> {selectedRequest.version}</p>
                    <p><strong>Minecraft:</strong> {selectedRequest.minecraft_release || "N/A"}</p>
                    ...
                    {selectedRequest.coordinates && (
                      <div>
                        ...
                        <p style={{ marginTop: "8px" }}>
                          X: {selectedRequest.coordinates.x}, Y: {selectedRequest.coordinates.y}, Z: {selectedRequest.coordinates.z}
                        </p>
                      </div>
                    )}
                  </div>
