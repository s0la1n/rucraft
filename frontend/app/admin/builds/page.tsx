...
import { buildModerationApi, BuildModerationRequest, getBackendBaseUrl } from "@/lib/api";
import "../admin.css";
...
function getImageUrl(imagePath: string | null): string {
  ...
  return `${getBackendBaseUrl()}/content-image/builds/${encodeURIComponent(filename)}`;
}
...
export default function AdminBuildsPage() {
  const [requests, setRequests] = useState<BuildModerationRequest[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [currentPage, setCurrentPage] = useState(1);
  const [lastPage, setLastPage] = useState(1);
  const [statusFilter, setStatusFilter] = useState("");
  const [selectedRequest, setSelectedRequest] = useState<BuildModerationRequest | null>(null);
  ...
  const loadRequests = async (page = 1, status = "") => {
    try {
      setLoading(true);
      setError(null);
      const response = await buildModerationApi.getBuilds({ per_page: 12, page, status });
  ...
  const handleApprove = async (id: number) => {
    try {
      setActionLoading(id);
      await buildModerationApi.approve(id, adminComment || undefined);
      await loadRequests(currentPage, statusFilter);
      setSelectedRequest(null);
      setAdminComment("");
    } catch (err) {
      alert(err instanceof Error ? err.message : "Не удалось одобрить сборку");
    } finally {
  ...
  const handleReject = async (id: number) => {
    ...
    try {
      setActionLoading(id);
      await buildModerationApi.reject(id, adminComment);
      await loadRequests(currentPage, statusFilter);
      setSelectedRequest(null);
      setAdminComment("");
    } catch (err) {
      alert(err instanceof Error ? err.message : "Не удалось отклонить сборку");
    } finally {
      setActionLoading(null);
    }
  };

  const openModal = (request: BuildModerationRequest) => {
  ...
  return (
    <RequireAuth adminOnly>
      <div className="page-content">
        ...
        <PageSection title="Модерация сборок">
          {/* Фильтры */}
            ...
            {requests.map((request) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="skin-card-category" style={{ fontSize: "12px" }}>Версия: {request.minecraft_version}</p>
                  <p className="skin-card-category" style={{ fontSize: "12px" }}>Сложность: {request.difficulty}</p>
              ...
              <div className="modal-body">
                <div style={{ display: "flex", gap: "20px", marginBottom: "20px" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p><strong>Версия Minecraft:</strong> {selectedRequest.minecraft_version}</p>
                    <p><strong>Сложность:</strong> {selectedRequest.difficulty}</p>
                      ...
                      <div>
                        ...
                        <ul style={{ marginTop: "8px", paddingLeft: "20px" }}>
                          {selectedRequest.materials.map((mat, idx) => (
                            <li key={idx}>{mat.name}: {mat.count}</li>
                          ))}
                        </ul>
                      </div>
                    )}
